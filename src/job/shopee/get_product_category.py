...
settings.setup_logging(f'/crawler/log/{current_time}-get_product_category.log')
...
class GetProductCategory:
    ...
    async def main(self, crawler_urls):
        ...
        async with aiohttp.ClientSession(
            ...
            logger.info(f"Added {len(updated_list_products)} products to updated_list_products")

            # Save to database
            await self.save(updated_list_products)
            logger.info(f"Updated {len(updated_list_products)} products to database successfully")
    ...
    async def fetch(self, client, query_url):
        try:
            ...
            logger.warning(f"Exception: {query_url} Fetch shop detail from shopee failed")
            return None


    async def save(self, updated_list_products):
        if len(updated_list_products) > 0:
            self.product_repository.update_all(updated_list_products, self.product_repository.get_product_by_item_id)
    ...
    async def parse_data(self, raw_data):
        try:
            ...
            logger.warning(f"Exception: {raw_data} Parse product detail from shopee failed")
            return None
