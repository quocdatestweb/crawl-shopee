...
class SaveProductStatistics:
    def __init__(self):
        ...
        self.product_timescale_repository = ProductTimescaleRepository(timescale_session)
    ...
    def __call__(self):
        ...
        while True:
            products = self.product_repository.get_list(limit, offset)
            if len(products) == 0:
                break
    ...
    async def save_product_statistics(self, products):
        ...
        for product in products:
            logger.info(f"Saving product statistics: {product.item_id} in shop {product.shop_id}")
