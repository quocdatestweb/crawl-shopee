...
settings.setup_logging(f"/crawler/log/{current_time}-get_shop_detail.log")
...
class GetShopDetail:
    ...
    def __call__(self):
        crawler_urls = []
        list_shop_ids = self.shop_repository.list_shop_ids()
        for shop_id in list_shop_ids:
            crawler_urls.append(f"{self.base_url}{shop_id.shop_id}")
        limit = 10
